...
The sample-compatible shortcut reads `features/auth/store/authStore` from `core/api/client.ts` to attach tokens and handle 401 logout. This is acceptable for small FE-only admin apps.

The preferred upgrade is callback injection: `core/` exposes `setApiClientAuth(...)`, and the auth feature wires token/logout into the client.
 ...
 * Preferred upgrade: use callback injection so core does not import features.
 */
